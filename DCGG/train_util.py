...
class TrainLoop:
    ...
    def _load_ema_parameters(self, rate):
        ema_params = copy.deepcopy(self.mp_trainer.master_params[:len(list(self.model.parameters()))])
    ...
    def run_loop(self):
        ...
        while (
            ...
            mask_semantics = input_label.scatter_(1, mask, 1.0)
            model_kwargs["mask"] = mask_semantics
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            for k, v in cond.items():
                if isinstance(v, list):
                    sublists = []
                    for sublist in v:
                        sublists.append(sublist[i : i + self.microbatch].to(dist_util.dev()))
                    micro_cond[k] = sublists
                else:
